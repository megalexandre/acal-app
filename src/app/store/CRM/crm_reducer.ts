...
export const CRMReducer = createReducer(
  ...
  on(updateContactSuccess, (state, { updatedData }) => {
    return {
      ...state,
      Contact: state.Contact.map((contacts) => (contacts._id === updatedData._id ? updatedData : contacts)),
      error: null,
    };
  }),
  ...
  on(updateCompanySuccess, (state, { updatedData }) => {
    return {
      ...state,
      Company: state.Company.map((Companys) => (Companys._id === updatedData._id ? updatedData : Companys)),
      error: null,
    };
  }),
